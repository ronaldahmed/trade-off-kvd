...
# CACHE_DIR = "/disk/ocean/rcardenas/tools/scibert" if getpass.getuser()=="s1987051" else \
# CACHE_DIR = "/home/s1987051/tools/scibert" if getpass.getuser()=="s1987051" else \
CACHE_DIR = "/disk/ocean/rcardenas/tools/scibert" if getpass.getuser()=="s1987051" else \
            "/lustre/home/sc066/roncar/tools/scibert/"
...
def get_model(model_type, num_layers, all_layers=None):
    if "t5" in model_type:
        ...
        model = T5EncoderModel.from_pretrained(CACHE_DIR, local_files_only=True)
    else:
        model = AutoModel.from_pretrained(CACHE_DIR, local_files_only=True)
    model.eval()
...
def get_tokenizer(model_type, use_fast=True):
    # if model_type.startswith("allenai/scibert"):
    #     model_type = cache_scibert(model_type)

    if LooseVersion(trans_version) >= LooseVersion("4.0.0"):
        tokenizer = AutoTokenizer.from_pretrained(CACHE_DIR, use_fast=use_fast,local_files_only=True)
    else:
...
# def cache_scibert(model_type, cache_folder="~/.cache/huggingface/transformers"):
#     if not model_type.startswith("allenai/scibert"):
#         return model_type
...
#         if not os.path.exists(json_file):
#             with open(json_file, "w") as f:
#                 print('{"do_lower_case": true, "model_max_length": 512, "init_inputs": []}', file=f)
